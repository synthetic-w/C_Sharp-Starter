...
static void BookPositionFinder(string bookName, string[,,] library)
{
    for (int i = 0; i < library.GetLength(0); i++)
    {
        for (int j = 0; j < library.GetLength(1); j++)
        {
            ...
            {
                if (library[i, j, k] != null && library[i, j, k].ToLower() == bookName.ToLower())
                {
                    Console.WriteLine($"Книга с названием {bookName} находится на {i + 1} стеллаже, на {j + 1} полке и стоит {k + 1} номером");
                }
            }
        }
    }
...
static void BookRandomizer(string[,,] library)
{
    ...
    int bookStillage = random.Next(library.GetLength(0) - 1);

    int bookShelf = random.Next(library.GetLength(1) - 1);

    int bookPosition = random.Next(library.GetLength(2) - 1);

    Console.WriteLine("Введите название книги:");
    string bookName = Console.ReadLine();

    if (library[bookStillage, bookShelf, bookPosition] == null)
    {
        library[bookStillage, bookShelf, bookPosition] = bookName;
    }
    else
    {
